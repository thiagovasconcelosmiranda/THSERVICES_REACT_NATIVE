...
`;
 ...
 const ModalSched = ({dat, authId, listAnnouncement, styleActive}) => {
  ...
  const handleButtonSchedule = async ()=>{
    ...
    if(newDate === '' || newTime === '' || descrition === ''){
      alertMessage('Empty space!', 'red');
        setPreloadButton(false);
       
    }else{
        const token = await AsyncStorage.getItem('token');
        if(token){
          const res = await Api.createSchedule(listAnnouncement.title, newDate, newTime, descrition, listAnnouncement.client_id, authId, 'Não confirmado', token);
          if(res.data !== ''){
            setPreloadButton(false);
